...
const blackBackdrop=document.querySelector(".black-drop");
...
const username=document.querySelector(".welcome .content h1");
const userNameValue=document.querySelector(".welcome .content p")
const savedUser=localStorage.getItem("user");
if(savedUser) {
  loginPage.style.display = "none";
  wrapper.style.display = "flex";
  username.innerHTML=`Hello ${savedUser}`;
}
...
loginBtn.addEventListener("click", () => {
  ...
  loginPage.style.transform="translateX(-150%)";
  loginPage.style.display="none";
  wrapper.style.display="flex";
...
// ];
let tasks=[];
let selctedCategory=categories[0];
...
const calculateTotal = () =>{
    const categoryTasks = tasks.filter(
      (task) => task.category.toLowerCase() === selctedCategory.title.toLowerCase()
    );
...
const renderTasks = () => {
  ...
  const categoryTasks = tasks.filter(
    ...
    categoryTasks.forEach((task) => {
      ...
      // add completion functionlity on clickbox
...
//lets add funcionality to add new tasks
...
// these are all are already stored tasks
getLocal();
calculateTotal();
renderCategories();
renderTasks();